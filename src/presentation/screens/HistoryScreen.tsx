...
import { GetServiceHistoryUseCase } from '../../domain/usecases/GetServiceHistoryUseCase';
...
const getHistory = new GetServiceHistoryUseCase(repo);
...
export default function HistoryScreen() {
  ...
  async function loadHistory() {
    const data = await getHistory.execute(user.id);
    setHistory(data);
    setLoading(false);
  }
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {history.length === 0 && !loading ? (
        <View style={styles.empty}>
          ...
          <Text style={styles.emptySubtext}>Usa el botón SOS cuando necesites ayuda</Text>
        </View>
      ) : (
        <FlatList
          data={history}
